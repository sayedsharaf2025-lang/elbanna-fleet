...
export const DeductionsTab: React.FC = () => {
  ...
  // كشف الخصومات المجمّع — كل خصم بيتسجل هنا وقت الحفظ، وبيتطبع مرة واحدة في نهاية الشاشة
  const [sessionDeductions, setSessionDeductions] = useState<Array<{
    driver_code: string;
    driver_name: string;
    amount: number;
    remaining: number;
  }>>([]);
  const [showBatchStatement, setShowBatchStatement] = useState(false);
  ...
  return (
    <div className="space-y-6" id="deductions_tab_root" style={{ direction: 'rtl' }}>
      ...
      {/* كشف الخصومات المجمّع — يطبع كل الخصومات المسجلة في الجلسة دفعة واحدة */}
      {showBatchStatement && (
        <A5PrintPreview
          type="batch_deductions"
          batchDeductionsData={{
            statementDate: new Date().toISOString().split('T')[0],
            entries: sessionDeductions
          }}
          onClose={() => setShowBatchStatement(false)}
        />
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
          ...
          <div className="overflow-x-auto text-xs font-sans">
              ...
              <tbody>
                {db.drivers
                  ...
                  .map(d => {
                    ...
                    // السيارة الأوتوماتيك — بترتيب الأقدم فالأحدث حسب تاريخ المخالفة (FIFO)
                    const driverViolationsSorted = db.violations
                      .filter(v => v.driver_id === d.id)
                      .sort((a, b) => (a.violation_date || '').localeCompare(b.violation_date || ''));
                    const autoCarNum = driverViolationsSorted[0]?.car_number;
                      ...
                      <tr key={d.id} className="border-b border-slate-100 hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3 px-3 font-bold text-slate-800">
                          <div className="flex items-center gap-1.5">
                            ...
                            <div>
                              <div>{d.name}</div>
                              {autoCar && <div className="text-[10px] text-indigo-500 font-mono" title="السيارة الأقدم في المخالفات يتم اختيارها تلقائياً">🚗 {autoCar.car_number} (الأقدم)</div>}
                            </div>
                          </div>
                        </td>
                          ...
                          <div className="flex gap-1.5 justify-center">
                            <button
                              ...
                              onClick={() => {
                                ...
                                // يُضاف للكشف المجمّع بدلاً من طباعة كشف مستقل لكل سائق —
                                // الطباعة تتم مرة واحدة في نهاية الشاشة لكل الخصومات المسجلة
                                setSessionDeductions(prev => [
                                  ...prev,
                                  { driver_code: d.driver_code, driver_name: d.name, amount: amt, remaining: newRemaining }
                                ]);
            ...
            {/* كشف الخصومات المجمّع — يظهر في نهاية الشاشة، ويطبع كل الخصومات المسجلة في الجلسة دفعة واحدة */}
            <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-slate-50 rounded-xl px-4 py-3">
              <div className="text-xs text-slate-600 font-semibold">
                {sessionDeductions.length === 0
                  ? 'لم يتم تسجيل أي خصومات في هذه الجلسة بعد'
                  : `تم تسجيل ${sessionDeductions.length} خصم في هذه الجلسة — إجمالي ${sessionDeductions.reduce((s, en) => s + en.amount, 0).toLocaleString()} ج.م`}
              </div>
              <div className="flex gap-2">
                {sessionDeductions.length > 0 && (
                  <button
                    type="button"
                    onClick={() => { if (confirm('مسح قائمة الخصومات المسجلة في هذه الجلسة؟ (لن يؤثر على الخصومات المحفوظة فعلياً في حساب السائقين)')) setSessionDeductions([]); }}
                    className="bg-white hover:bg-slate-100 text-slate-600 font-bold px-3 py-2 rounded-xl border border-slate-200 transition-all text-xs"
                  >
                    مسح القائمة
                  </button>
                )}
                <button
                  type="button"
                  disabled={sessionDeductions.length === 0}
                  onClick={() => setShowBatchStatement(true)}
                  className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold px-4 py-2 rounded-xl transition-all text-xs shadow-sm"
                >
                  <Printer className="w-3.5 h-3.5" />
                  طباعة كشف الخصومات ({sessionDeductions.length})
                </button>
              </div>
            </div>
          </div>
